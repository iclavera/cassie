...
class CassieVis:
    ...
    def get_image(self):
        # Right now it jsut works with the default windows size
        image = gl.glReadPixels(0, 0, 1200, 1200, gl.GL_RGB, gl.GL_UNSIGNED_INT)[900::-1, :, :]
        return image

    def draw(self, c):
        vis = cassie_vis_draw(self.v, c.c)
        if self._record_video:
            self._buffer.append(self.get_image())
        return vis
