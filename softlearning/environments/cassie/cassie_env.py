...
class CassieEnv(gym.Env, utils.EzPickle):
    ...
    def _cassie_state_to_obs(self, int_state, state):
        ...
        if False:
            ...
            # pelvis
            pelvis_ori = np.array(
                quaternion_to_euler(np.array(int_state.pelvis.orientation).astype(np.float64)))  # TODO: Change to euler
            pelvis_pos = np.array(int_state.pelvis.position)
            ...
            qpos = np.array(
                [motor_pos[0], motor_pos[1], motor_pos[2], motor_pos[3], joint_pos[0], joint_pos[1], motor_pos[4],
                 motor_pos[5], motor_pos[6], motor_pos[7], motor_pos[8], joint_pos[3], joint_pos[4], motor_pos[4]])

            qvel = np.array(
                [motor_vel[0], motor_vel[1], motor_vel[2], motor_vel[3], joint_vel[0], joint_vel[1], motor_vel[4],
        ...
        # state randomization
        obs = obs + np.random.normal(0, 0.01, size=len(obs))
        # state randomization
    ...
    def reset(self):
        ...
        qvel = qvel + np.random.uniform(low=-0.01, high=0.01, size=len(qvel))
    ...
    def do_simulation(self, u, n_frames):
        ...
        for _ in range(n_frames):
            internal_state_obj = self.sim.step_pd(
                u)  # step_pd returns state_out_t structure -> however this structure is still not fully understood
    ...
    def reward(self, internal_state, state, action):
        ...
        stability_cost = self.stability_cost_coef * 0.5 * np.mean(
            np.square(pelvis_transl_vel[1:]))  # quadratic velocity of pelvis in y and z direction ->
        ...
        if self.task == 'balancing':
            ...
            reward = 1.0 * np.exp(-100.0 * vel_cost) \
                     ...
                     + 1.0 * np.exp(-100.0 * stability_cost) \
                     + 1.0 * np.exp(-100.0 * impact_cost)
            # print(np.exp(-10* vel_cost), np.exp(- 10*ctrl_cost), np.exp(- 10*stability_cost), np.exp(- 10*impact_cost))
    ...
    def set_default_parameters(self):
        self.parameters = dict(cassie_torque_limits=np.array([4.5 * 25, 4.5 * 25, 12.2 * 16, 12.2 * 16, 0.9 * 50]),
                               # ctrl_limit * gear_ratio
                               cassie_motor_vel_limits=np.array([2900, 2900, 1300, 1300, 5500]) / 60 / (2 * np.pi),
                               # max_rpm / 60 / 2*pi
                               p_gain_range=[10, 10000],
                               d_gain_range=[1, 100],
