...
//    auto mag = rot.RotateVector(worldMag);
    ...
    auto worldAccel = pelvisPtr_->WorldLinearAccel() - worldPtr_->Gravity();
    auto worldGyro = pelvisPtr_->WorldAngularVel();
    auto worldVel = pelvisPtr_->WorldLinearVel();
    auto worldMag = ignition::math::Vector3d(0, 1, 0);

    auto rot = pose.Rot().Inverse();
    auto accel = worldAccel;
    auto vel = worldVel;
    auto gyro = worldGyro;
    auto mag = worldMag;
...
//    cassieOut_.pelvis.vectorNav.magneticField[0] = pose.Pos().X;
//    cassieOut_.pelvis.vectorNav.magneticField[1] = pose.Pos().Y;
//    cassieOut_.pelvis.vectorNav.magneticField[2] = pose.Pos().Z;

    // Set accelerometer/gyro/magnetometer measurement
    for (size_t i = 0; i < 3; ++i) {
        cassieOut_.pelvis.vectorNav.linearAcceleration[i] = vel[i];
        cassieOut_.pelvis.vectorNav.angularVelocity[i] = gyro[i];
        cassieOut_.pelvis.vectorNav.magneticField[i] = pose.Pos()[i];
    }
}
